...
async def amba_handler(message: Message):
    ...
    if chk == 1:
        ...
        await bd_handler(message, text)
        await message.answer(
             f"Это раздел «Амбассадоры» \n\n В нем ты можешь найти амбассадоров предыдущих наборов, амбассадоров из твоего города или просто найти спикера на мероприятие!",
             keyboard=(
                Keyboard()
...
async def event_handler(message: Message):
    ...
    if chk == 1:
        ...
        await bd_handler(message, text)
        await message.answer(
             "Это раздел «Мероприятия» \n\n В нем ты можешь найти гайды для мероприятий или подчерпнуть идеи для своих мероприятий!",
             keyboard=(
                Keyboard()
...
async def number_handler(message: Message):
    ...
    if chk == 1:
        await message.answer(
             f"Это раздел с категориями мероприятий. \n\n В нем ты можешь выбрать любое мероприятие и получить информацию о нем. \n \n Можно получить мероприятия по типу формата: \n 🔴 – Любой \n🟠 – Онлайн \n🔵 – Оффлайн \n \n Или перейти во вкладку тип и выбрать конкретное.",
             keyboard=(
                Keyboard()
